...
Start the recording process with your robot configuration file:

```bash
python -m lerobot.rl.gym_manipulator --config_path <your_env_config.json>
```
...
```bash
python -m lerobot.rl.learner --config_path <your_train_config.json>
```
...
```bash
python -m lerobot.rl.actor --config_path <your_train_config.json>
```
