...
def update_meta_data(
    ...
    # have more data files than the merged result (e.g. 8 source files → 2 merged files).
    data_src_to_dst = data_idx.get("src_to_dst", {})
    if data_src_to_dst:
        orig_chunk = df["data/chunk_index"].astype(int).tolist()
        orig_file = df["data/file_index"].astype(int).tolist()
        dst_chunks = []
        dst_files = []
        for c, f in zip(orig_chunk, orig_file):
            dst_c, dst_f = data_src_to_dst.get((c, f), (data_idx["chunk"], data_idx["file"]))
            dst_chunks.append(dst_c)
            dst_files.append(dst_f)
        df["data/chunk_index"] = dst_chunks
        df["data/file_index"] = dst_files
    else:
...
def aggregate_data(src_meta, dst_meta, data_idx, data_files_size_in_mb, chunk_size):
    ...
    Returns:
        dict: Updated data_idx with current chunk and file indices, plus a
              'src_to_dst' dict mapping source (chunk, file) → destination (chunk, file).
    """
    ...
    # Track which source (chunk, file) maps to which destination (chunk, file).
    # This is needed so update_meta_data() can correctly remap each episode's
    # data/chunk_index and data/file_index to the actual merged file locations.
    src_to_dst = {}
    ...
    for src_chunk_idx, src_file_idx in unique_chunk_file_ids:
        ...
        src_schema = pq.read_schema(src_path)
        ...
        # After append_or_create_parquet_file returns, data_idx["chunk"/"file"]
        # points to the destination file that received this source file's data.
...
def _df_to_table(df: pd.DataFrame, src_schema: pa.Schema) -> pa.Table:
    """Convert DataFrame to Arrow Table using src_schema, stripping pandas metadata.

    pa.Table.from_pandas() injects pandas metadata (numpy_type=object for list columns)
    which confuses the HuggingFace dataset viewer. We keep only the original HF metadata
    from src_schema so the viewer correctly recognises action/observation.state as
    Sequence[float32] and renders line charts.
    """
    table = pa.Table.from_pandas(df, schema=src_schema)
    orig_meta = {k: v for k, v in (src_schema.metadata or {}).items() if k != b"pandas"}
    return table.replace_schema_metadata(orig_meta)
...
def append_or_create_parquet_file(
    ...
    aggr_root: Path = None,
    src_schema: pa.Schema | None = None,
):
    ...
    if not dst_path.exists():
        ...
        elif src_schema is not None:
            pq.write_table(_df_to_table(df, src_schema), dst_path)
        else:
    ...
    if contains_images:
        ...
        pq.write_table(_df_to_table(final_df, src_schema), target_path)
    else:
