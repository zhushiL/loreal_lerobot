...
class DobotNova5(Robot):
    ...
    def _send_joint_movj(self, joint_degrees: list[float]) -> str:
        if self._robot is None:
            raise DeviceNotConnectedError(f"{self} is not connected.")
        # Explicit joint-space command. Avoids controller-side IK ambiguity.
        command = "JointMovJ(" + ",".join(f"{float(value):.6f}" for value in joint_degrees) + ")"
        return self._robot.sendRecvMsg(command)

    def _send_movj_legacy(self, joint_degrees: list[float]) -> str:
        if self._robot is None:
            raise DeviceNotConnectedError(f"{self} is not connected.")
        # Some Nova5 firmware variants accept only positional MovJ syntax:
        # MovJ(j1,j2,j3,j4,j5,j6)
        command = "MovJ(" + ",".join(f"{float(value):.6f}" for value in joint_degrees) + ")"
        return self._robot.sendRecvMsg(command)
    ...
    def _move_joint_movj(self, joint_degrees: list[float], description: str) -> None:
        ...
        response = self._send_joint_movj(target)
        self.logger.info(f"{description} JointMovJ response: {response.strip()}")
        error_id, values = self._parse_dobot_response(response)
        command_name = "JointMovJ"
        if error_id != 0:
            # Older controller firmware may not expose JointMovJ via dashboard API.
            # In that case, fall back to legacy MovJ positional syntax.
            if error_id == -10000:
                self.logger.warn(
                    "JointMovJ is not supported by this controller firmware, "
                    "falling back to MovJ(j1..j6) compatibility syntax."
                )
                response = self._send_movj_legacy(target)
                self.logger.info(f"{description} MovJ response: {response.strip()}")
                values = self._raise_if_dobot_error(response, "MovJ")
                command_name = "MovJ"
            else:
                self._raise_if_dobot_error(response, "JointMovJ")
                return
