...
from franky import Robot as FrankyRobot, JointMotion, CartesianMotion, ReferenceType, CartesianVelocityMotion, Twist, JointVelocityMotion, Duration, Affine
...
class FrankaResearch3(Robot):
    ...
    def connect(self, calibrate: bool = True, go_to_start: bool = True) -> None:
        ...
        try:
            ...
            # Set relative dynamics factor for safe operation
            self.robot.relative_dynamics_factor = self.config.dynamics_factor
            logger.info(f"Set dynamics factor to {self.config.dynamics_factor}")
