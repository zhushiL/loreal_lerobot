    ...
    def get_action(self, current_tcp_pose_quat: np.ndarray | None = None) -> dict[str, Any]:
        ...
        Args:
            current_tcp_pose_quat: Optional 8D actual TCP pose
                ...
                a grip-press drift guard fires: if Python's cached `_target_quat`
                differs from the actual TCP by more than
                `config.target_tcp_drift_max_deg`, REF_RESET is aborted and the
                cached target is resynced to the actual TCP. When None, behavior
                is unchanged. The steady-state envelope against Flexiv's 90°
                orientation-error fault (301005) is enforced on the robot side
                in BiFlexivRizon4RT.send_action, not here — that way the
                teleop's internal target tracking stays a faithful record of
                user intent, and the clamp only modifies the action dict that
                flows to the hardware (and into the recorded dataset).
