...
from lerobot.teleoperators.pico4_hand.config_pico4_hand import Pico4HandConfig
...
class Pico4Hand(Teleoperator):
        ...
        try:
            ...
            for attempt in range(max_retries):
                if self.config.use_right_controller:
                    ...
                    hand_state = xrt.get_left_hand_tracking_state()
                    pose = hand_state[0]
                    pose_has_data = bool(xrt.get_left_hand_is_active()) and any(abs(v) > 1e-6 for v in pose)
                else:
            ...
            # Initialize start pose (will be updated when control is first enabled)
            self._start_pos = current_tcp_pose_quat[:3].copy()

            # Initialize reference/offset (will be set when control is first enabled)
            self._ref_pos = None
    ...
    def get_action(self) -> dict[str, Any]:
        ...
        if self.config.use_right_controller:
            ...
            pose = hand_state[0]
            left_hand_active = bool(self._xrt.get_left_hand_is_active())
            controller_grip = 1.0 if left_hand_active else 0.0
        ...
        # Step 2: Check enable state.
        # - Right controller mode: grip acts as an enable switch with hysteresis.
        # - Left hand tracking mode: enable immediately whenever the hand is actively tracked.
        ...
        if self.config.use_right_controller:
            ...
            self._enabled = left_hand_active
        if self._enabled != prev_enabled:
            if self.config.use_right_controller:
                ...
                self.logger.debug(
                    f"[ENABLE] State changed: {prev_enabled} -> {self._enabled}, "
                    f"left_hand_active={left_hand_active}"
                )
